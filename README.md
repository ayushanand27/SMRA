...
| **Known CVEs in transitive dependencies** | `pypdf` fixed (bumped `<6.0.0` → `<7.0.0`, verified: full test suite + a live RAG query still pass). The rest are pinned deliberately, confirmed by testing, not just left idle: `pillow≥12` breaks `streamlit==1.41.1` (`pillow<12` required) and `sentence-transformers 5.x`/`transformers 5.x` fail to import at runtime on this stack (`tokenizers` ABI mismatch) even though pip's resolver reports no conflict — caught by the semantic-cache tests, not by `pip-audit` or `pip check`. `aiohttp≥3.11` forces `langchain-pinecone≥0.2.7`, which drags `langchain-core` to the `1.x` line and breaks `langchain`/`langchain-community`/`langchain-huggingface`/`langgraph` (all pinned `<1.0.0`) — the `langchain-core`/`langchain-text-splitters` CVE fixes only exist in that `1.x` line, so clearing them needs a full LangChain 1.0 migration, not a patch bump. Tracked via `pip-audit` (CI, report-only) and Dependabot |
...
| **No dependency lockfile** | `smra/requirements*.txt` use version ranges, not a hash-pinned lockfile. A same-machine attempt to generate one hit two walls: a project-local venv can't be created here (Windows 260-char path limit, given how long this repo's parent path is) and this machine's shared global Python environment pulls in unrelated packages (`gradio`, `ultralytics`, etc.) that pollute a plain `pip freeze`. Needs a clean environment (shorter path or Linux/WSL) — good candidate for a dedicated CI job that publishes the lockfile as an artifact |
